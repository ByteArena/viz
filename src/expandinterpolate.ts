...
export default function (next3: [Frame, Frame, Frame], tps: number, targetfps: number, sendFrame: (message: Object) => void) {
    ...
    const synthesizeFrame = function (framenum) {
        ...
        objectDiffs.map(agentdiff => {
            ...
            interpolatedframe.Objects[index].Position = thisFrameObjectHash[agentdiff.id].position.clone().add(
                agentdiff.position.clone().mult(timeratio)
            ).toArray();

            interpolatedframe.Objects[index].Orientation = thisFrameObjectHash[agentdiff.id].orientation + (agentdiff.orientation * timeratio);
        });
