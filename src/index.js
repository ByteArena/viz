...
import {
    observeStoreUpdateFrameBatch,
    observeStoreUpdateGameSettings
} from "./app/observers/game";
...
function initpc(dispatch: StoreDispatch) {
    ...
    function onSceneLoaded(/* scene */) {
        ...
        comm(settings.wsurl, settings.tps, (type: string, data: any) => {
            switch (type) {
                case "status": {
                ...
                case "init": {
                    ...
                    data.agents.forEach(agent => {
                        dispatch(
                            actions.agent.addAgent(agent.AgentName, agent.Id),
                        );
                    });
...
observeStoreUpdateFrameBatch(store, () => game);
